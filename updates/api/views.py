...
class UpdateModelDetailAPIView(HttpResponseMixin, CSRFExemptMixin, View):
	...
	def get_object(self, id=None):
		...
		""" Upper one is also a method to account if an object does not exist. Lower one is a more efficient django way of taking things into account"""
	...
	def post(self, request, *args, **kwargs):
		json_data = json.dumps({"message": "Not Allowed, use endpoint"})
...
class UpdateModelListAPIView(HttpResponseMixin, CSRFExemptMixin, View):
	...
	def put(self, request, *args, **kwargs):
		...
		if not passed_id:
			error_data = json.dumps({"id": "this id is a required field to update an item"})
			return self.render_to_response(error_data, status=400)
